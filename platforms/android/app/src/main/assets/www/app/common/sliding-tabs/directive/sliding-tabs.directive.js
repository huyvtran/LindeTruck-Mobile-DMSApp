    ...
    return {
        ...
        link: function (scope, element, attrs, parent) {
            ...
            var init = function () {
                ...
                if (angular.isDefined(attrs.parentId) && attrs.parentId !== "") {
                    options.parentId = attrs.parentId;
                }

                if (angular.isDefined(attrs.parentClass) && attrs.parentClass !== "") {
                    options.parentClass = attrs.parentClass;
                }
                ...
                if (options.parentId !== '') {
                    var parentIdElement = angular.element(document.getElementById(options.parentId));
                    parentIdElement.prepend(tabsBar);
                } else if (options.parentClass !== '') {
                    var parentClassElement = angular.element(document.getElementsByClassName(options.parentClass));
                    parentClassElement.after(tabsBar);
                }
                else {
                    slider.parent().after(tabsBar);
                }
            ...
            var slideToCurrentPosition = function () {
                ...
                var targetLeftOffset = targetTab.prop("offsetLeft") - 10;
...
slidingTabsDirective.directive('ionSlideTabLabel', [function () {
    return {
        ...
        link: function ($scope, $element, $attrs, $parent) {
            console.log('IONDIRECTIVE ', $attrs.ionSlideTabLabel);
