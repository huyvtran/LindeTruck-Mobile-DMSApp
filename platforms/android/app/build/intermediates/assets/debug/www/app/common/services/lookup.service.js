(function () {
    ...
    /**
     * Module oinio.common.services LookupService
     */
    angular.module('oinio.common.services')
        .service('LookupService', function LookupService($q, $log) {
            ...
            this.searchLookup = function (objectType, searchTerm, searchField, recordTypeDeveloperName) {
                ...
                if (objectType === 'User') {
                    smartQuery = 'SELECT {User:_soup} FROM {User} LEFT OUTER JOIN {Contact} ON {User:ContactId} = {Contact:Id} WHERE {Contact:AVANTO_User_Role__c} = \'Standard\'';

                    // WHERE clause for search term
                    if (searchTerm) {
                        smartQuery = smartQuery + ' AND {' + objectType + ':' + searchField + '} LIKE \'%' + searchTerm + '%\'';
                    }
                }
                else {
                    smartQuery = 'SELECT {' + objectType + ':_soup} FROM {' + objectType + '}';
