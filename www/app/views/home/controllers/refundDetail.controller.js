(function () {
  ...
  angular.module('oinio.controllers')
    .controller('RefundDetailController',
      function ($scope, $rootScope, $filter, $state, $ionicPopup, $ionicHistory, $stateParams, AppUtilService,
                ForceClientService, ConnectionMonitor,
                LocalCacheService) {
        ...
        $scope.goToSave = function () {
          ...
          var payload = $scope.paramSaveDeliveryOrdersUrl + "?deliveryorders=" + JSON.stringify(
            $scope.selectRefundInfo);
        ...
        $scope.goToSaveAndSubmit = function () {
          ...
          var payload1 = $scope.paramSaveDeliveryOrdersUrl + "?deliveryorders=" + JSON.stringify(
            $scope.selectRefundInfo) + "&recordId=" + $stateParams.orderDetailsId;
          // console.log("payload", payload);
          ForceClientService.getForceClient().apexrest(payload1, 'POST', {}, null, function (response) {
            console.log("POST_success1:", response);
            var payload2 = $scope.paramSaveAndSubminDeliveryOrdersUrl + "?recordId=" + $stateParams.orderDetailsId
