(function () {
  ...
  angular.module('oinio.controllers')
    .controller('TransferController', function ($scope, $rootScope, $ionicPopup, $filter, $log, $state, $stateParams,
                                                ...
                                                AppUtilService, dualModeService) {
      var vm             = this,
          currentOrderId = null,
      ...
      $scope.initPageAllWokers = function () {
        ...
        $scope.allWorkers = [];
        dualModeService.getWorkOrderUtilInfo(Number(localStorage.onoffline), currentOrderId,
              ...
              setTimeout(function () {
                if (res.soResult != undefined && res.soResult != null) {
                  if (res.soResult.Service_Order_Owner__c != undefined && res.soResult.Service_Order_Owner__c != null) {
                    $('#selectUserGroup').find('option[value =' + res.soResult.Service_Order_Owner__c + ']').attr(
                      'selected', true);
                  } else {
                    if (res.soResult.Service_Order_Owner__r != undefined && res.soResult.Service_Order_Owner__r != null
                        && res.soResult.Service_Order_Owner__r.Id != undefined && res.soResult.Service_Order_Owner__r.Id
                        != null) {
                      $('#selectUserGroup').find('option[value =' + res.soResult.Service_Order_Owner__r.Id + ']').attr(
                        'selected', true);
                    }
                  }
                }
