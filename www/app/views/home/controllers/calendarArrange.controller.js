angular.module('oinio.CalendarArrangeController', [])
    .controller('CalendarArrangeController', function ($scope, $ionicPopup, $stateParams, HomeService, $state, $rootScope,
        ...
        $scope.submitOrder = function () {
            ...
            //         });
            //new code
            dualModeService.updateServiceOrderOverviewStatusUtil(Number(localStorage.onoffline),Number(localStorage.onoffline)!==0?$stateParams.workOrderId:$stateParams.SendSoupEntryId, 'Not Started').then(function callBack(res) {
                console.log(res);
                //AppUtilService.hideLoading();
                if (res.status.toLowerCase() == "success") {
                    ForceClientService.getForceClient().apexrest(
