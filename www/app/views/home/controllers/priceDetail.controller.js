    ...
    function ($scope, $rootScope, $ionicPopup, $ionicHistory, $filter, $log, $state, $stateParams, Service1Service,
        ...
        ForceClientService.getForceClient().apexrest(maintenanceKeyLevelPartssBy2Url, 'GET', {}, null,
          function (response) {
            ...
            var getPartsRelatedsUrl = $scope.partsRelatedsUrl + JSON.stringify(parts_number__cList) + '&partsQuantitys='
                                      + JSON.stringify(partsQuantitys) + '&accountId=' + $scope.basicInfo.Ship_To__c+'&Rate__c='+$scope.Rate__c+'&Pricing_Date__c='+$scope.Pricing_Date__c+'&isRental='+$scope.isRentalInfo+'&isDMS='+ true;
              ...
              function (responsePartsRelateds) {
                for (let i = 0; i < responsePartsRelateds.length; i++) {
                  ...
                  if (truckFitItem.priceCondition) {
                    ...
                    partsItem.SPN_Discount__c = truckFitItem.priceCondition.spnDiscount;
          ...
          if (element[0].checked) {
            ...
            if (!existFlag) {
              ...
              if (ele.priceCondition) {
                ...
                partsItem.SPN_Discount__c = ele.priceCondition.spnDiscount;
                partsItem.SPN_Price__c = ele.priceCondition.spnPrice;
              } else {
                partsItem.Gross_Price__c = '';
              }
      ...
      $scope.addLabourOriginalsList = function (obj) {
        ...
        for (let index = 0; index < $scope.selectedTruckFitItems.length; index++) {
          ...
          oneLabourOriginals4['SPN_Discount__c'] = selectedTruckFitItemsIndex.SPN_Discount__c; //spn折扣
          oneLabourOriginals4['SPN_Price__c'] = selectedTruckFitItemsIndex.SPN_Price__c; //spn价
