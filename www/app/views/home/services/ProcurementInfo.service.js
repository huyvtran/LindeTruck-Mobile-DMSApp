(function() {
    ...
    angular
        ...
        .service('ProcurementInfoService', function($q, $filter, LocalDataService, ConnectionMonitor, IonicLoadingService, LocalSyncService) {
                    ...
                    navigator.smartstore.runSmartQuery(querySpec, function (cursor) {
                        ...
                        if (cursor && cursor.currentPageOrderedEntries && cursor.currentPageOrderedEntries.length) {
                            angular.forEach(cursor.currentPageOrderedEntries, function (entry) {
                                result['Province__c'] = entry[0].Mobile_Offline_Name__c;
                                result['City__c'] = entry[0].Work_Order_Type__c;
                                result['Address__c'] = entry[0].Description__c;
                                result['Post_Code__c'] = entry[0].Service_Suggestion__c;
                                result['Telephone__c'] = entry[0].Description__c;
                                result['FAX__c'] = entry[0].Service_Suggestion__c;
