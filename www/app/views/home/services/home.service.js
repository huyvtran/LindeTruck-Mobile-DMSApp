(function() {
    ...
    angular.module('oinio.services', [])
      .service('HomeService',
        function HomeService($q, $filter, $log, LocalDataService, ConnectionMonitor, IonicLoadingService,
                             LocalCacheService, LocalSyncService, ForceClientService, SMARTSTORE_COMMON_SETTING) {
            let service = this;
            ...
            */
            ...
            this.getAccountForAccIds = function (Ids) {
                ...
                if (Ids == null || Ids.accIds == null || Ids.orders == null || Ids.accIds.length < 1
                    || Ids.orders.length < 1) {
                ...
                let querySpec = navigator.smartstore.buildSmartQuerySpec(sql,
                  SMARTSTORE_COMMON_SETTING.PAGE_SIZE_FOR_ALL);
                navigator.smartstore.runSmartQuery(querySpec, function (cursor) {
                    let account;
                    if (cursor && cursor.currentPageOrderedEntries && cursor.currentPageOrderedEntries.length) {
                        angular.forEach(cursor.currentPageOrderedEntries, function (entry) {
            ...
            this.addServiceOrders = function (adrs, trucks) {
                ...
                if (adrs != null && adrs[0] != null && adrs[0].Service_Order_Type__c != null
                    && adrs[0].Service_Order_Type__c != '') {
                ...
                LocalDataService.createSObject('Service_Order_Overview__c', str_recTypeDevName).then(
                  function (sobject) {
                      var newItem, adr;
                      var adrsToSave = [];
                      var isError = false;
                      ...
                      for (var i = 0; i < adrs.length; i++) {
                          ...
                          if ((adr.Account_Ship_to__c == '' || adr.Account_Ship_to__c == null)
                              && (adr.Account_Ship_to__r._soupEntryId == '' || adr.Account_Ship_to__r._soupEntryId
                                  == null)) {
                              var isError = true;
                              continue;
                          }
                          ...
                          if (adr.Service_Order_Type__c != null && adr.Service_Order_Type__c
                              != '') {newItem['Service_Order_Type__c'] = adr.Service_Order_Type__c;}
            ...
            this.modifyWorkOrder = function (order, user, day) {
                ...
                LocalDataService.getSObject('Service_Order_Overview__c', order._soupEntryId).then(function (sobject) {
                    ...
                    if (sobject['Status__c'] == null || sobject['Status__c'] == '' || sobject['Status__c']
                        == 'Not Planned') {
            ...
            this.modifyChildWorkOrder = function (parentsid, user, day) {
                ...
                service.searchChildOrderForParent(parentsid).then(function (orders) {
                    angular.forEach(orders, function (order) {
                        ...
                        if (order['Status__c'] == null || order['Status__c'] == '' || order['Status__c']
                            == 'Not Planned') {
            ...
            this.init20AcctTrucks = function (acctId) {
                ...
                let sql = "select {Truck_Fleet__c:_soup}\
                         from {Truck_Fleet__c}\
                         where {Truck_Fleet__c:Ship_To_CS__c} ='" + acctId
                          + "' order by {Truck_Fleet__c:Name} asc limit 150";
