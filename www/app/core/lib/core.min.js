angular.module("oinio.core", ["oinio.settings", "ionic-datepicker"]).config(["$sceDelegateProvider", "$sceProvider", function ($sceDelegateProvider, $sceProvider) {
    ...
    angular.module("oinio.core").service("LocalSyncService", ["$injector", "$q", "$log", "$http", "ConfigurationService", "ForceClientService", "LocalCacheService", "APP_SETTINGS", "MetaService", "LocalDataService", "DescribeService", "SMARTSTORE_COMMON_SETTING", "RecordTypeService", "PicklistService", "RestService", "ConnectionMonitor", "SalesforceDataService", "FileService", "SYNC_PAGE_SIZE", "UtilService", function ($injector, $q, $log, $http, ConfigurationService, ForceClientService, LocalCacheService, APP_SETTINGS, MetaService, LocalDataService, DescribeService, SMARTSTORE_COMMON_SETTING, RecordTypeService, PicklistService, RestService, ConnectionMonitor, SalesforceDataService, FileService, SYNC_PAGE_SIZE, UtilService) {
        ...
        service.syncBusinessSObjects = function (option) {
            ...
            console.log('current object sync wrapper:::', objectSyncWrapper);
            var deferred = $q.defer();
            return console.log(">>>> start sync process beginning.", new Date()),$log.debug(">>>> start sync process beginning."), service.startSyncUpObjects(objectSyncWrapper).then(function (objectSyncResult) {
                console.log(">>>> sync up ended, start to sync down.", new Date()), $log.debug(">>>> sync up ended, start to sync down."), deferred.notify(objectSyncResult), service.startSyncDownObjects(objectSyncWrapper).then(function (objectSyncResult) {
                    console.log(">>>> sync down ended, start to clean up.", new Date()),$log.debug(">>>> sync down ended, start to clean up."), deferred.notify(objectSyncResult), service.startCleanUpObjects(objectSyncWrapper).then(function (objectSyncResult) {
                        console.log(">>>> clean up ended, start sync down file data.", new Date()),$log.debug(">>>> clean up ended, start sync down file data."), deferred.notify(objectSyncResult), service.startSyncDownFileData().then(function () {
                            console.log(">>>> start sync down attachment data.", new Date()),$log.debug(">>>> start sync down attachment data."), service.startSyncDownAttachmentBody().then(function () {
                                console.log(">>>> start rebuild reference.", new Date()), $log.debug(">>>> start rebuild reference."), service.startRebuildLocalReferenceForObjects(objectSyncWrapper).then(function (objectSyncResult) {
                                    console.log(">>>> rebuild reference ended, finished all sync process.", new Date()), $log.debug(">>>> rebuild reference ended, finished all sync process."), deferred.resolve(objectSyncResult)
                                }, function (error) {
                                    deferred.reject(error)
                                }, function (process) {
                                    deferred.notify(process)
        ...
        var syncDownSubsidiaryRecords = function (objectType, subsidiaryObjectName, queryStr, lastSyncDownDate, serverTimestamp) {
            ...
            console.log('current time :::', param);
