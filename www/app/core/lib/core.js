...
(function (angular) {
    ...
    angular.module('oinio.core.error', [])
        ...
        .config(function ($httpProvider, $provide) {
            ...
            $httpProvider.interceptors.push(function ($q, $rootScope, $injector, HTTP_DEFAULT_ERROR_MSG, HTTP_NETWORK_ERROR_MSG) {

                // cache previous routing state
                $rootScope.$on('$stateChangeSuccess', function (event, toState, toParams, fromState, fromParams) {
                    if (fromState.name) {
                        $injector.get('LocalCacheService').set('previousRoutingState', {
            ...
            $provide.decorator('$exceptionHandler', function ($delegate, $injector, APP_SETTINGS, EXCEPTION_SEVERITY) {
                ...
                var redirectRoute = function () {
                    ...
                    $injector.get('MetaService').getMetaValueEnhance('initialized').then(function (appInitialized) {
                        if (appInitialized) {
                            ...
                            var previousState = $injector.get('LocalCacheService').get('previousRoutingState');
                            var invalidBackStates = ['app.synchronize', 'app.password', 'app.switch-user', 'app.forgot-password', 'app.new-password', 'app.change-password',
                                'synchronize', 'password', 'login', 'switch-user', 'forgot-password', 'new-password'];

                            // 1. When current state is at root/first page whose state is '', redirect to the start view.
                            // 2. When current state is at synchronize page, and previous state page is null, root/first, or some user login related page(invalid back states),
                            //    redirect to the start view; otherwise go back to previous state page.
                            // 3. When current state is not root/first page and synchronize page, do nothing to stay at current state page.
                            if (!currentStateName) {
                                $state.go($injector.get('APP_SETTINGS').START_VIEW);
                            } else if (currentStateName === 'app.synchronize' || currentStateName === 'synchronize') {
                                if (!previousState || invalidBackStates.indexOf(previousState.stateName) !== -1) {
                                    $state.go($injector.get('APP_SETTINGS').START_VIEW);
                                } else {
                ...
                return function (exception, cause) {
                    ...
                    var okLabel = UtilService.translate('mobile_global_btn_ok', 'cl.global.btn_ok');
                    okLabel = (okLabel === 'cl.global.btn_ok') ? 'OK' : okLabel;

                    var retryLabel = UtilService.translate('mobile_global_btn_retry', 'cl.global.btn_retry');
                    retryLabel = (retryLabel === 'cl.global.btn_retry') ? 'RETRY' : retryLabel;

                    var cancelLabel = UtilService.translate('mobile_global_btn_cancel', 'cl.global.btn_cancel');
                    cancelLabel = (cancelLabel === 'cl.global.btn_cancel') ? 'CANCEL' : cancelLabel;

                    // handle INVALID_SESSION_ID error
    ...
    angular.module('oinio.core')
        .service('DescribeService', function ($q, ForceClientService, MetaService, $log, $http, $cordovaFile, SMARTSTORE_COMMON_SETTING, UtilService, $timeout, LocalCacheService, Exception) {
            ...
            service.getDescribeSObjects = function (clearCache) {
                ...
                if (describeSObjectsCache && describeSObjectsCache.length > 0 && !clearCache) {
                ...
                navigator.smartstore.querySoup('_describeLayout', querySpec, function (cursor) {
                        for (var i = 0; i < cursor.currentPageOrderedEntries.length; i++) {
                            ...
                            for (var j = 0; j < describeLayoutResult.describeLayouts.length; j++) {
                                ...
                                if (describeLayout.recordTypeId === recordTypeId || (recordTypeId == null && describeLayout.master === true)) {
    ...
    angular.module('oinio.core')
        .service('LocalDataService', function ($q, $http, $filter, $log, SMARTSTORE_COMMON_SETTING, APP_SETTINGS, LocalCacheService, PicklistService, DescribeService, ConnectionMonitor, FileService, MetaService, UtilService, $injector) {
                            ...
                            _getRecordTypeByDeveloperName(objectType, recordType).then(function (recordTypeEntry) {
                                ...
                                __resolveObjectFieldDefaultValue(objectType, recordTypeDeveloperName, newSObjectRecord).then(function (resolvedObject) {
                                    ...
                                    if (recordTypeEntry) {
                                        ...
                                        resolvedObject['RecordTypeId' + SMARTSTORE_COMMON_SETTING.REFERENCED_FIELD_EXTENSION_SUFFIX] = recordTypeEntry._soupEntryId;
                                        resolvedObject['RecordTypeId' + SMARTSTORE_COMMON_SETTING.REFERENCED_FIELD_TYPE_SUFFIX] = 'RecordType';
            ...
            service.queryConfiguredObjectByName = function (objectName) {
                return $q(function (resolve, reject) {
                    if (!objectName) {
                        reject(new Error('LocalDataService.queryConfiguredObjectByName error: missing parameter.'));
                    } else {
...
(function (angular, _) {
    ...
    angular.module('oinio.core')
        .service('LocalSyncService', function ($injector, $q, $log, $http, ConfigurationService, ForceClientService,
                                               ...
                                               RestService, ConnectionMonitor, SalesforceDataService, FileService, SYNC_PAGE_SIZE, UtilService) {
                var service = this;
                ...
                /**
                 * synchroniue meta date for configured object types
                 * @param objectSyncWrapper
                 */
                service.syncConfiguredObjectsMeta = function (objectSyncWrapper) {
                    ...
                    LocalDataService.queryConfigurationAndObjects(_currentUser.ProfileId)
                        .then(function (config) {
                            configuration = config;

                            // Update FilterCriteria in record synchronisation
                            return _updateObjectFilterCriteria(configuration.objects);
                        })
                        .then(function () {
                            // create object type names array
                            for (var i = 0; i < configuration.objects.length; i++) {
                                objectTypeNames.push(configuration.objects[i].Name);
                            }
                        ...
                        .then(function (describeResults) {
                            ...
                            // sarch for object types where we need to synchronize all RecordTypes
                            var allRecordTypes = [];
                            for (var rt = 0; rt < configuration.objects.length; rt++) {
                                if (configuration.objects[rt]['MobileVizArt__All_RecordTypes__c'] && configuration.objects[rt]['MobileVizArt__All_RecordTypes__c'] === true) {
                                    allRecordTypes.push(configuration.objects[rt].Name);
                                }
                            }

                            // synchronize RecordTypes
                ...
                service.syncForLanguageChange = function () {
                    ...
                    MetaService.getMetaValue('objectPermissions')
                        ...
                        .then(function (config) {
                            ...
                            for (var obj in objectPermissions) {
                                if (objectPermissions.hasOwnProperty(obj) && objectPermissions[obj]['PermissionsRead'] === true) {
                                    filteredObjectTypeNames.push(obj);
                                }
                            }

                            // Load describe for each sobject, including object attributes, record types, fields, picklist values, etc.
                            return DescribeService.loadDescribeSObjects(filteredObjectTypeNames, true);
                        })
                        .then(function (describeResults) {
                            describeSObjectResults = describeResults;
                        ...
                        .then(function (result) {
                            if (APP_SETTINGS.ENABLE_ONLINE_TRANSLATION) {

                                // load custom labels through VF page, and refresh the translation
                                return $injector.get('TranslationService').loadCustomLabels();
                            }
                        })
                        .then(function () {
                            deferred.resolve(true);
                 ...
                 * @param {[]} [allRecordTypes] array with object types for which all record types should be synchronized
                 * @returns {*}
                 */
                var syncRecordTypes = function (describeSObjectResults, allRecordTypes) {
                    ...
                    for (var i = 0; i < describeSObjectResults.length; i++) {
                        var describeSObjectResult = describeSObjectResults[i];

                        for (var j = 0; j < describeSObjectResult.recordTypeInfos.length; j++) {
                            var recordTypeInfo = describeSObjectResult.recordTypeInfos[j];

                            if ((recordTypeInfo.available === true || (allRecordTypes && allRecordTypes.indexOf(describeSObjectResult.name) !== -1)) && recordTypeIds.indexOf(recordTypeInfo.recordTypeId) === -1) {
                ...
                service.startSyncDownObjects = function (objectSyncWrapper) {
                    ...
                    checkObjectTypesBeforeSynchronize(objectSyncWrapper).then(function () {
                ...
                service.startCleanUpObjects = function (objectSyncWrapper) {
                        ...
                        if (index >= objectSyncWrapper.cleanUpObjects.objectTypes.length) {
                            ...
                            if (contentDocumentObjectType && contentVersionObjectType) {
                                ...
                                service.cleanUpAllContentDocument().then(function (cleanUpResults) {
                                    ...
                                    if (!objectSyncWrapper.cleanUpObjects.noNeedLastCleanupDate) {
                                        MetaService.setMetaValue('lastCleanUpDate', serverTimestamp || new Date().toISOString());
                                        //IonicLoadingService.hide();
                                    }
                    ...
                    RestService.getServerTimestamp().then(function (serverTimestamp) {
                        ...
                        if (!objectSyncWrapper.cleanUpObjects.forceCleanup) {
                            ...
                            checkCleanUpInterval(serverTimestamp).then(function (cleanupTimeUp) {
                                if (cleanupTimeUp) {

                                    // Start to clean up the first object
                                    syncSequential(0, serverTimestamp);
                    ...
                    if (sidArray && sidArray.length > 0) {
                        ...
                        navigator.smartstore.retrieveSoupEntries('Attachment', sidArray, function (entries) {
                            ...
                            if (entries.length > 0) {
                                ...
                                if (attachment.hasOwnProperty('ParentId_type')) {
                                    ...
                                    // cleanup is needed we go foreword
                                    deferred.resolve(true);
                                }
                    ...
                    if (syncResults.success) {
                        ...
                        angular.forEach(records, function (record, index) {
                            ...
                            if (syncResult.isSuccess) {
                                ...
                                $log.log('Error during synchronization to Salesforce: ' + JSON.stringify(queueMessage));
                            }
                        });
                ...
                var checkObjectTypesBeforeSynchronize = function (objectSyncWrapper) {
                    ...
                    var deferred = $q.defer();
                    var objectTypes = objectSyncWrapper.syncDownObjects.objectTypes;
                    ...
                    fetchCheckedObjectTypes(objectTypeNames).then(function (onlineObjectsInfo) {
                        ...
                        if (onlineObjectsInfo) {
                            ...
                            // According to whether the current user having new sharing updated,
                            // if having, and the sharing time after last sync time, set forceCleanup is true
                            if (onlineObjectsInfo.sharingUpdatedTimeValue) {
                                MetaService.getMetaValue('lastCleanUpDate').then(function (lastCleanUpDate) {
                                    if (lastCleanUpDate && new Date(onlineObjectsInfo.sharingUpdatedTimeValue) > new Date(lastCleanUpDate)) {
                                        objectSyncWrapper.cleanUpObjects.forceCleanup = true;
                                    }
                ...
                var fetchCheckedObjectTypes = function (objectTypeNames) {
                    return $q(function (resolve, reject) {
                        ...
                        for (var i = 0; i < objectTypeNames.length; i++) {
                            ...
                            getObjectLastSyncDownDatePromiseArr.push(
                                MetaService.getObjectMetaByType(objName).then(function (meta) {
                                    ...
                                    } else if (meta && skipSyncDownObjects.indexOf(meta.name) === -1) {
                        ...
                        $q.all(getObjectLastSyncDownDatePromiseArr).then(function () {
                            ...
                            if (!_lastSyncDownDate || hasNullLastSyncDownDate) {
                                resolve(null);
                                return;
                            }
                                    ...
                                    if (configuration) {
                                        ...
                                        var parseResp = function (response) {
                                            ...
                                            if (response) {
                                                ...
                                                result.sharingUpdatedTimeValue = response.sharingUpdatedTimeValue;
                                        ...
                                        if (configuration['MobileVizArt__Cleanup_Online_Field__c']) {
                                            postBody.clsRequest.sharingUpdatedTimeField = configuration['MobileVizArt__Cleanup_Online_Field__c'];
                                        }
                ...
                /**
                 * Clean up some specific objects.
                 *
                 * @param {Array} [objectNames] the objects name array for clean up
                 * @returns {Promise}
                 */
                service.cleanUpObjectsByNames = function (objectNames) {
                    ...
                    var handleError = function (error) {
                        if (error && typeof error.handle === 'function') {
                            error.retry = service.cleanUpObjectsByNames;
                            error.retryContext = service;
                            error.retryParam = objectNames;
                            error.retryDeferred = deferred;
                            error.handle();
                        } else {
                            new Exception(EXCEPTION_SEVERITY.RECOVERABLE, error.status, error.message, error.stack, error, service.cleanUpObjectsByNames, null, service, objectNames, deferred).handle();
                        }
                        deferred.reject(error);
                    };
                    ...
                    ConfigurationService.objectTypesToSynchronize().then(function (objectTypes) {
                        ...
                        for (var i = 0; i < objectTypes.length; i++) {
                            var objectType = objectTypes[i];
                            if (!objectNames || objectNames.indexOf(objectType.name) !== -1 || skipCleanUpObjects.indexOf(objectType.name) !== -1) {
                                obj2CleanUp.push(objectType);
                                objectType.needSyncUp = false;
                                objectType.needSyncDown = false;
                                objectType.needCleanUp = true;
                            }
                        }

                        // initialize for sync up objects
                        objectSyncWrapper.syncUpObjects.objectTypes = [];
                        objectSyncWrapper.syncUpObjects.syncProgress = service.initializeSyncUpProgress([]);
                        ...
                        // initialize for clean up objects, clean up job should check all object types to clear up any dirty records.
                        objectSyncWrapper.cleanUpObjects.objectTypes = obj2CleanUp;
                        objectSyncWrapper.cleanUpObjects.forceCleanup = true;
                        objectSyncWrapper.cleanUpObjects.noNeedLastCleanupDate = true;
                        objectSyncWrapper.cleanUpObjects.syncProgress = service.initializeCleanUpProgress(obj2CleanUp);
...
(function (angular) {
    ...
    angular.module('oinio.core')
        .service('RestService', function ($q, $log, ForceClientService, LocalCacheService, LocalDataService, MetaService, APP_SETTINGS, $injector) {
            ...
            var getCustomLabelsUrl = function () {
                if (APP_SETTINGS.LOCAL_CONFIGURATION === true) {
                    return '/MobileVizArt/loadCustomLabels'; // url without namesapece
                } else {
                    return '/MobileVizArt/MobileVizArt/loadCustomLabels'; // url with namesapece
                }
            };
             ...
             * String timeStamp;
             * String sharingUpdatedTimeField;
             * String confId;
             * Array objectConfig;
             * e.g:
             * {
             ...
             *     "sharingUpdatedTimeField": "Sharing_Updated_Time__c",
             *     "confId": null,
             *     "objectConfig": [{
             *       "name": "Account",
             *       "lookupModStamp": null,
             ...
             *  "sharingUpdatedTimeValue":"2017-04-13T03:51:35.888Z",
             *  "objectTypes":[{
             *      "parent":null,
             *      "objectType":"Account",
             *      "filterCriteria":"Name != null",
            ...
            /**
             * @ngdoc method
             * @name getCustomLabels
             * @methodOf oinio.core.service:RestService
             * @description
             * Get custom labels in salesforce by their names.
             *
             * @param {object} payload - parameter
             * String customLabelNames - it's all necessary custom label names connected with comma;
             * e.g:
             * {
             *     "customLabelNames": "test_label_2,test_label"
             * }
             *
             * @returns {promise} array records
             * e.g:
             * {
             *     "test_label_2,test_label": "\r\n<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" \"http://www.w3.org/TR/html4/loose.dtd\">\n<html>
             *         <head><script src=\"/static/111213/js/perf/stub.js\" type=\"text/javascript\"></script></head>
             *         <body>\n    ==customLabelRender-start==test 2@#@test@#@==customLabelRender-end==</body></html>"
             * }
             */
            service.getCustomLabels = function (payload) {
                ...
                // call custom REST class in Mobile VizARt package
                ForceClientService.getForceClient().apexrest(getCustomLabelsUrl(), 'POST', JSON.stringify(payload), null, function (response) {
                    $log.debug('>>>> RestService getCustomLabels() success');
                    deferred.resolve(response);
                }, function (error) {
                    var msg = error['responseJSON'] && error['responseJSON'][0]['message'] ? error['responseJSON'][0]['message'] : null;
                    deferred.reject(new Exception(EXCEPTION_SEVERITY.RECOVERABLE, error.status, msg, error.stack, error));
...
(function (angular) {
     ...
     */
    angular.module('oinio.core')
        .service('SalesforceDataService', function ($q, $log, ForceClientService, UtilService, MetaService, LocalDataService, RestService, Exception, EXCEPTION_SEVERITY, LocalCacheService) {
            var service = this;
            ...
            /**
             * Add file share member for some specified routing records according to setting.
             *
             * @param {string} fileId file's Id(ContentDocument Id for target file object)
             * @param {string} shareType the type of share (V - View, C - Collaboration)
             *
            ...
            service.addFileShareMemberByRouting = function (fileId, shareType) {
                ...
                _getMatchRouting()
                    .then(function (targetCriteria) {
                        return _queryMultiTargetObjects(targetCriteria);
                    })
                    .then(function (targetIds) {
                        var promises = [];
                        for (var i = 0; i < targetIds.length; i++) {
                            promises.push(service.addFileShareMember(fileId, targetIds[i], shareType));
                        }
                        return $q.all(promises);
                    })
                    .then(function (res) {
                    ...
                    .catch(function (error) {
                        if (typeof error === 'string') {
                            error = {message: 'Add file share member by routing error: ' + error};
                        }
                        deferred.reject(new Exception(EXCEPTION_SEVERITY.RECOVERABLE, error.status, error.message, error.stack, error));
                    });
            ...
            /**
             * Get routing setting matched the current user according to the configuration and the current user information.
             * JSON configuration such as:
             * {
             *   "criteriaObject": "User", // currently, it only supports User, Profile and Permission object
             *   "criteriaObjectField": "Country__c", // field can be any of User object, and Name of Profile and Permission object
             *   "routing": [ // routing is an array containing the mapping for different values of the criteria object field value
             *     {
             *       "en": [ // one file can be uploaded to multiple records as this is an array
             *         {
             *           "logObject": "Log_Object__c", // this could be any object
             *           "logObjectField": "Country__c", // this could be any field of the logObject
             *           "logObjectFieldValue": "en" // this is the value of the logObjectField to query, the result record is the record which the log file will be uploaded to
             *         },
             *         {
             *           "logObject": "Log_Object__c",
             *           "logObjectField": "Country__c",
             *           "logObjectFieldValue": "en_US"
             *         }
             *       ]
             *     },
             *     {
             ...
             *           "logObject": "Log_Object__c",
             *           "logObjectField": "Country__c",
             *           "logObjectFieldValue": "de"
             *         }
             *       ]
             *     }
             *   ],
             *   "defaultRouting": { // if no routing is found above, then use this as default place to upload
             *     "logObject": "Log_Object__c",
             *     "logObjectField": "Country__c",
             *     "logObjectFieldValue": "en"
             *   }
             * }
             *
             * @returns {*|promise}
            ...
            var _getMatchRouting = function () {
                ...
                LocalDataService.queryConfigurationAndObjects().then(function (configuraion) {
                    ...
                    var routingSetting = configuraion['Log_File_Upload_Routing__c'];
                    if (routingSetting) {
                        var userInfo = LocalCacheService.get('userInfo');
                        var currentVal = '';
                        ...
                        var pushTargetRecords = function (setting, me) {
                            if (setting.hasOwnProperty(me) && setting[me]) {
                                for (var i = 0; i < setting[me].length; i++) {
                                    var perRecords = setting[me][i];
                                    var targetObj = perRecords['logObject'];
                                    if (targetCriteria.hasOwnProperty(targetObj)) {
                                        targetCriteria[targetObj] += ' or ';
                                    } else {
                                        targetCriteria[targetObj] = '';
                                    }
                                    targetCriteria[targetObj] += perRecords['logObjectField'] + ' = \'' + perRecords['logObjectFieldValue'] + '\'';
                                }
                            }
                        };

                        if (currentVal) {
                            for (var i = 0; i < routingSetting['routing'].length; i++) {
                                var item = routingSetting['routing'][i];

                                if (typeof currentVal === 'string') {
                                    pushTargetRecords(item, currentVal);
                                } else {
                                    for (var j = 0; j < currentVal.length; j++) {
                                        pushTargetRecords(item, currentVal[j]);
                                    }
                                }
                            }
                        }

                        if (Object.keys(targetCriteria).length === 0 && routingSetting['defaultRouting']) {
                            pushTargetRecords({'default': [routingSetting['defaultRouting']]}, 'default');
                        }
                    }
            ...
            /**
             * Query all ids of more than one objects according to the target records filter criteria.
             * These ids will be used for the log files new share target.
             *
             * @param {object} targetCriteria target object name and its filter criteria map
             *
             * @returns {*|promise}
             */
            var _queryMultiTargetObjects = function (targetCriteria) {
                var deferred = $q.defer();
                ...
                if (promises.length > 0) {
                    $q.all(promises).then(function (resultWrap) {
                        var ids = [];

                        for (var i = 0; i < resultWrap.length; i++) {
                            ids = ids.concat(_.pluck(resultWrap[i].records, 'Id'));
                        }

                        if (ids.length > 0) {
                            deferred.resolve(ids);
                        } else {
                            deferred.reject({message: 'You can\'t access any records for criteria:' + JSON.stringify(targetCriteria) + '.'});
                        }
                    }, function (error) {
                        deferred.reject(error);
                    });
                } else {
                    deferred.reject({message: 'Not found routing setting matched for you.'});
                }
...
(function (angular) {
     ...
     */
    angular.module('oinio.core')
        .service('SalesforceLoginService', function (ForceClientService, SalesforceDataService, $http, $q, LocalCacheService, MetaService, LocalDataService, FileService, UtilService, APP_SETTINGS, ConnectionMonitor, $log, Exception, EXCEPTION_SEVERITY) {
            var service = this;
            ...
            service.logout = function () {
                ...
                FileService.clearUserFolder().then(function () {
                    // polyfill for android platform
                    if (UtilService.isAndroidOS()) {
                            // if there is only one user logined in the App, remove the _user soup
                        var smartsql = 'select {_user:_soup} from {_user}'; 
                        var smartquery = navigator.smartstore.buildSmartQuerySpec(smartsql, 100);
                        navigator.smartstore.runSmartQuery(true, smartquery, function(cursor){
                            if (cursor && cursor.totalEntries <= 1) {
                                navigator.smartstore.removeSoup(true, '_user', function (cursor) {
                                    doLogout();
                                }, function (err) {
                                    throw err;
                                });
                            } else if (cursor && cursor.totalEntries > 1) {
                                // as we will use the number of users in _user soup to determin remove the soup or not, we need
                                // to remove the record here
                                var currentUser = LocalCacheService.get('currentUser');
                                var exactQuery = navigator.smartstore.buildExactQuerySpec('username', currentUser['Username']);
                                navigator.smartstore.querySoup(true, '_user', exactQuery, function (cursor) {
                                    ...
                                    if (currentUserEntryId) {
                                        navigator.smartstore.removeFromSoup(true, '_user', [currentUserEntryId], function (succeed) {
                                            doLogout();
                                        }, function (err) {
                                            throw err;
                                        });
            ...
            service.oauthLogin = function (callback, failCallBack) {
                ...
                var processBusiness = function (credentials, clientId, callback) {
                ...
                oauthPlugin.getAuthCredentials(
                    ...
                    function (credentials) {
                        ...
                        // for mobilesdk 6.0+ the clientId never exported by the credentials for android platform,
                        // we will load it from local configuration here
                        var clientId = credentials.clientId;
                        if (!clientId) {
                            var sdkinfoPlugin = cordova.require('com.salesforce.plugin.sdkinfo');
                            sdkinfoPlugin.getInfo(function (sdkinfo) {
                                if (sdkinfo && sdkinfo.bootConfig) {
                                    clientId = sdkinfo.bootConfig.remoteAccessConsumerKey;
                                }

                                processBusiness(credentials, clientId, callback);
                            }, failCallBack);
                        } else {
                            processBusiness(credentials, clientId, callback);
                        }
    ...
    angular.module('oinio.core')
        ...
        .service('SmartStoreService', function ($injector, $q, SMARTSTORE_SOUP_STATUS, ConfigurationService, APP_SETTINGS, MetaService, LocalSyncService, LocalCacheService, UtilService) {
            ...
            service.setupSoups = function () {
                ...
                setupGlobalSoups()
                    ...
                    .then(function () {
                        ...
                        return LocalSyncService.syncMobileConfiguration();
                    })
                    .then(function (result) {
                        if (APP_SETTINGS.ENABLE_ONLINE_TRANSLATION) {
                ...
                navigator.smartstore.soupExists(isGlobal, soupName, function (exists) {
                    ...
                    if (!exists) {
                        ...
                        if (isExternalStorage && typeof navigator.smartstore.registerSoupWithSpec === 'function' && UtilService.isAndroidOS()) {
                            navigator.smartstore.registerSoupWithSpec(isGlobal, {
                                name: soupName,
            ...
            service.dropSoupByConfig = function (skipObjectSoups, skipFrameworkSoups, skipGlobalSoups) {
                ...
                // drop business soups
                service.dropBusinessSoups(skipObjectSoups).then(function () {

                    // drop framework soups
                    return service.dropFrameworkSoups(skipFrameworkSoups);
                }).then(function () {

                    // todo: drop for global if needed.
            ...
            service.dropFrameworkSoups = function (skipFrameworkSoups) {
                ...
                var removePromises = [];
                ConfigurationService.frameworkSoupSettings().then(function (soupSettings) {
                    for (var i = 0; i < soupSettings.length; i++) {
                        var soup = soupSettings[i];
                        removePromises.push(removeSoup(false, soup.name));
                    }
            ...
            service.dropBusinessSoups = function (skipObjectSoups) {
                ...
                var removePromises = [];
                navigator.smartstore.soupExists(false, '_configuration', function (exists) {

                    if (exists) {
                        $injector.get('LocalDataService').getConfiguredObjects().then(function (objects) {

                            for (var i = 0; i < objects.length; i++) {
                                var objectName = objects[i].Name;
                                removePromises.push(removeSoup(false, objectName));
                            }

                            // the AttachmentBody soup is not configured
                            removePromises.push(removeSoup(false, 'AttachmentBody'));
...
(function (angular) {
    ...
    /**
     * Module oinio.core TranslationService
     */
    angular.module('oinio.core')
        .service('TranslationService', function ($q, $log, $http, $translate, LocalCacheService, RestService, FileService, APP_SETTINGS) {
            var service = this;

            /**
             * Load custom labels from salesforce, then store them into i18n file.
             */
            service.loadCustomLabels = function () {
                var deferred = $q.defer();

                var localCustomLabels;

                // get the config custom label names
                $http.get(APP_SETTINGS.ONLINE_TRANSLATION_CONFIGURATION_FILE)
                    .then(function (fileContent) {
                        localCustomLabels = fileContent.data;

                        var customLabelNames = [];
                        for (var labelName in localCustomLabels) {
                            if (localCustomLabels.hasOwnProperty(labelName)) {
                                customLabelNames.push(labelName);
                            }
                        }

                        // get custom labels from salesforce
                        return getCustomLabels(customLabelNames);
                    })
                    .then(function (result) {
                        // store the custom labels result into the i18n file
                        if (result === -1) {

                            // if no config custom labels or paring failed, copy the local default content into the i18n file
                            return FileService.rewriteI18NData('locale-', localCustomLabels);
                        } else {

                            // store the online custom labels into the i18n file
                            return FileService.rewriteI18NData('locale-', result);
                        }
                    })
                    .then(function () {
            ...
            /**
             * Get custom labels from salesforce through calling rest service. Then parse the returned result
             * which is the multiple visualforce pages content including custom labels.
             * If no config custom label or paring failed, return -1.
             *
             * @param {Array} customLabelNames
             * @returns {*|promise}
             */
            function getCustomLabels(customLabelNames) {
                var deferred = $q.defer();

                if (customLabelNames && customLabelNames.length > 0) {

                    // get the value of the custom labels through calling rest service
                    RestService.getCustomLabels({customLabelNames: customLabelNames.join(',')}).then(function (result) {
                        var customLabelsMap = {};

                        // parse the html content of the returned multiple visualforce pages, to get the value of the custom labels
                        var startFlag = '==customLabelRender-start==';
                        var endFlag = '==customLabelRender-end==';
                        var splitFlag = '@#@';

                        for (var batchLabelNames in result) {
                            if (result.hasOwnProperty(batchLabelNames)) {
                                var htmlContent = result[batchLabelNames];

                                if (htmlContent.indexOf(startFlag) === -1 || htmlContent.indexOf(endFlag) === -1) {
                                    var logMsg = 'Can\'t parse the custom labels from the visualforce page content ' +
                                        'which is not exist the start flag or the end flag. The html content is \n' + htmlContent;
                                    void 0;
                                    $log.warn(logMsg);
                                    deferred.resolve(-1);
                                    return;
                                } else {
                                    htmlContent = htmlContent.substring(htmlContent.indexOf(startFlag) + startFlag.length, htmlContent.indexOf(endFlag));
                                    if (htmlContent.endsWith(splitFlag)) {
                                        htmlContent = htmlContent.substring(0, htmlContent.lastIndexOf(splitFlag));
                                    }

                                    // decode the html content
                                    htmlContent = htmlDecode(htmlContent);

                                    var labels = htmlContent.split(splitFlag);
                                    var labelNames = batchLabelNames.split(',');
                                    if (labels.length === labelNames.length) {

                                        for (var i = 0; i < labelNames.length; i++) {
                                            customLabelsMap[labelNames[i]] = labels[i];
                                        }

                                    } else {
                                        $log.debug('Parse failed: it is maybe existing a conflict with the split flag \"' + splitFlag + '\".');
                                        deferred.resolve(-1);
                                        return;
                                    }
                                }
                    ...
                    $log.debug('No any config custom labels.');
                    deferred.resolve(-1);
                }
            ...
            /**
             * Decode html content, like &#20013;&#25991; decoded into the readable content.
             *
             * @param {string} text the html which will be decoded.
             *
             * @returns {string}
             */
...
(function (angular) {
     ...
     */
    angular.module('oinio.core')
        .service('UtilService', function ($q, $http, $log, $injector, ForceClientService) {
            var service = this;
            ...
            /**
             * Translate label with custom label(salesforce) or local label according to online translation enabled.
             *
             * @param {string} customLabelName
             * @param {string} localLabelName
             * @returns {string}
             */
            service.translate = function (customLabelName, localLabelName) {
                ...
                if (APP_SETTINGS.ENABLE_ONLINE_TRANSLATION) {
                    if ($filter('translate')(customLabelName) !== customLabelName) {
                        return $filter('translate')(customLabelName);
                    } else {
                        return $filter('translate')(localLabelName);
                    }
                } else {
                    return $filter('translate')(localLabelName);
                }
    ...
    angular.module('oinio.core.components')
        .component('actionFooterBar', {
            ...
            controller: function ($scope, $http, $ionicPopup, $log, $state, PageLayoutService, ModalService,
                                  IonicLoadingService, LocalDataService, LocalSyncService, ConnectionMonitor, UtilService) {
                var ctrl = this;
                ...
                function synchronize(objectType) {
                    if (ConnectionMonitor.isOnline()) {
                        IonicLoadingService.show(UtilService.translate('mobile_sync_lb_synchronizing', 'cl.sync.lb_synchronizing'));
                ...
                this.editDetails = function () {
                    ...
                    PageLayoutService.generatePageLayoutForSObject(ctrl.sobject, true).then(function (pageLayoutResult) {
                        ...
                        ModalService.show('app/core/modal/templates/edit.record.modal.view.html', 'SaveModalController as vm', {
                            ...
                            if (save === true) {
                                IonicLoadingService.show(UtilService.translate('mobile_sync_lb_saving', 'cl.sync.lb_saving'));
        ...
        .service('Logger', function ($filter, $injector) {
            ...
            function logObject(exception, params) {
                ...
                if (typeof exception.handle === 'function' || exception.severity) {
                    ...
                    // addition original message from the http response returned by salesforce
                    if (exception.message && rawEx && rawEx['responseJSON'] && rawEx['responseJSON'][0]['message']) {
                        msg += ' [##### original message: ' + rawEx['responseJSON'][0]['message'] + ' #####]';
                    }
        ...
        .controller('ObjectHomeController', function ($scope, $filter, $stateParams, $ionicNavBarDelegate,
            ...
            vm.search = function (searchTerm) {
                ...
                if (searchTerm && searchTerm.trim().length < 3) {
                    vm.message = UtilService.translate('mobile_global_msg_searchTermLengthRequired', 'cl.global.msg_searchTermLengthRequired');
                } else {
                    LocalDataService.globalSearch(vm.objectType, searchTerm).then(function (resolve) {
                        searchResults = resolve;
                        vm.showingRecords = searchResults.slice(0, pageSize);
                        if (!resolve || !resolve.length) {
                            vm.message = UtilService.translate('mobile_global_msg_searchWithNoResults', 'cl.global.msg_searchWithNoResults').trim() + ' \'' + searchTerm + '\'';
                        }
                    });
                }
            };
    ...
    .component('multiPicklistComponent', {
        ...
        controller: function ($filter, UtilService) {
            ...
            var _getSelectedLabel = function(selected) {
                ...
                {
                    output = numOfElements + ' ' + UtilService.translate('mobile_global_lb_items', 'cl.global.lb_items');
                }
                return output;
            }
...
angular.module('oinio.core.pageLayoutRenderer')
    .component('referenceComponent', {
        ...
        controller: function ($scope, $filter, $sce, localizeService, LocalDataService, SimpleModalService, DescribeService, UtilService) {
            var _this = this;
            ...
            var initModalScope = function () {
                ...
                _this.search = function (searchTerm) {
                    ...
                    if (!searchTerm || (searchTerm && searchTerm.trim().length < 3)) {
                        _this.message = UtilService.translate('mobile_global_msg_searchTermLengthRequired', 'cl.global.msg_searchTermLengthRequired');
                    } else {
                        ...
                        LocalDataService.getFilterConfigByObjectType(_this.sobject.attributes.type).then(function(filterConfigByObjectType){
                            ...
                            if (filterConfigByObjectType.length) {
                                ...
                                LocalDataService.globalSearch(_this.objectType, searchTerm, limitSize, null, 'ASC', filterConfig).then(function (resolve) {
                                    ...
                                    cordova.plugins.Keyboard.close();
                                    if (!resolve || !resolve.length) {
                                        _this.message = UtilService.translate('mobile_global_msg_searchWithNoResults', 'cl.global.msg_searchWithNoResults').trim() + ' \'' + searchTerm + '\'';
                                    }
                                });
                            } else {
